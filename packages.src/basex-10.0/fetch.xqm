...
(:~ 
 : Fetches the resource referred to by the given <code>href</code> string and returns it as <a href="https://docs.basex.org/wiki/Lazy_Module">lazy</a> <code>xs:base64Binary</code> item.
 :
...
(:~ 
 ...
 :)
declare function fetch:text($href as xs:string) as xs:string external;
...
(:~ 
 ...
 : @error fetch:encoding the specified encoding is not supported, or unknown.
 :)
declare function fetch:text($href as xs:string, $encoding as xs:string) as xs:string external;
...
(:~ 
 ...
 : @error fetch:open the URI could not be resolved, or the resource could not be retrieved.
 :)
declare function fetch:doc($href as xs:string, $options as map(*)?) as document-node() external;
...
(:~ 
 ...
 : @error fetch:open the input could not be parsed.
 :)
declare function fetch:binary-doc($input as xs:anyAtomicType, $options as map(*)?) as document-node() external;

(:~ 
 : Returns the content-type (also called mime-type) of the resource specified by <code>href</code> string: <ul><li>If a remote resource is addressed, the request header will be evaluated.</li><li>If the addressed resource is locally stored, the content-type will be guessed based on the file extension.</li></ul>
 :
 : @param $href value of type xs:string
 : @return value of type xs:string
 : @error fetch:open the URI could not be resolved, or the resource could not be retrieved.
 :)
declare function fetch:content-type($href as xs:string) as xs:string external;
