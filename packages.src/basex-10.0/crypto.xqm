(:~ 
 ...
 : @see https://docs.basex.org/wiki/Cryptographic_Module
 :)
...
(:~ 
 ...
 : @param $type value of type xs:string
 : @param $ext1 value of type item()
 : @param $ext2 value of type node()
 ...
 :)
declare function crypto:generate-signature($input as node(), $canonicalization as xs:string, $digest as xs:string, $signature as xs:string, $prefix as xs:string, $type as xs:string, $ext1 as item(), $ext2 as node()) as node() external;
