...
(:~ 
 : Fetches the JSON document referred to by the given <code>$href</code> and converts it to an XQuery value. The <code>$options</code> argument can be used to control the way the input is converted.
 :
 ...
 :)
declare function json:doc($href as xs:string) as item()? external;
...
(:~ 
 ...
 : @error json:options the specified options are conflicting.
 :)
declare function json:doc($href as xs:string, $options as map(*)?) as item()? external;

(:~ 
 : Converts the JSON <code>$value</code> to an XQuery value. If the input can be successfully parsed, it can be serialized back to the original JSON representation. The <code>$options</code> argument can be used to control the way the input is converted.
 :
 ...
 : @error json:options the specified options are conflicting.
 :)
declare function json:parse($value as xs:string?) as item()? external;
...
(:~ 
 ...
 : @error json:options the specified options are conflicting.
 :)
declare function json:parse($value as xs:string?, $options as map(*)?) as item()? external;
