(:~ 
 : This <a href="https://docs.basex.org/wiki/Module_Library">XQuery Module</a> contains functions for extracting internal information about modules and functions and generating documentation.
 :
 : @author BaseX team (wiki scrape by quodatum/xq-catalogs) 
 : @see https://docs.basex.org/wiki/Inspection_Module
 :)
module namespace inspect = "http://basex.org/modules/inspect";
declare namespace xqdoc = "http://www.xqdoc.org/1.0";

(:~ 
 : Returns function items for all user-defined functions (both public and private) that are known in the current query context. If an <code>$href</code> value is specified, the specified resource will be retrieved as a string and compiled, and its functions will be added to the query context and returned to the user. A relative URI will be resolved against the static base URI of the query.
 :
...
(:~ 
 ...
 : @error inspect:parse Error while parsing a module.
 :)
declare function inspect:functions($href as xs:string) as function(*)* external;
...
(:~ 
 ...
 : @return value of type xs:string
 :)
declare function inspect:type($input as item()*, $options as map(*)) as xs:string external;
